...
function mapSobreaviso(item: any): Sobreaviso {
  return {
    ...
    totalHorasAtendimento: Number(item.total_horas_atendimento ?? item.totalHorasAtendimento ?? 0),
    intervalos: asArray(item.intervalos).map((intervalo: any) => ({
      id: Number(intervalo.id_intervalo ?? intervalo.id),
      sobreavisoId: Number(intervalo.id_sobreaviso ?? intervalo.sobreavisoId),
      tipo: intervalo.tipo,
      inicio: intervalo.inicio,
      fim: intervalo.fim,
      idOcorrencia: intervalo.id_ocorrencia ?? intervalo.idOcorrencia ?? null,
      observacao: intervalo.observacao ?? undefined,
    })),
    status: item.status,
    origem: item.origem,
